...
const MaterialSelection = ({onExportPDF}) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await api.get(`/combined/${id}`, {
          headers: {
  ...
  const submitMaterialSelection = async () => {
    ...
    try {
        ...
        // axios automatically throws an error for non-2xx responses, no need for manual check
        const data = response.data;
  ...
  const saveChanges = async (itemIndex, componentIndex) => {
    ...
    try {
      const response = await api.put(`/combined-details/${id}`,
        { items: updatedItems }, // The request body with updatedItems
        {
          headers: {
            'Content-Type': 'application/json',
            'x-auth-token': token,
      ...
      const response = await axios.put(
        ...
        { items: updatedItems },  // Passing the request body as the second argument
        {
          headers: {
            ...
            'x-auth-token': token,  // Authorization token required for the request
          },
        }
      );
      
      // axios automatically throws an error for non-2xx status codes,
      // so no need for a manual check like response.ok
      const data = response.data; 
